...
describe("Parse keyword when doing search", () => {
  ...
  it("Should parse name criteria first if it's not provided", (done) => {
    ...
    const expectedResult =
      "(name:(Nottingham~ yatch~ race~))^3 AND (boat_names:(limay~))^2";
    expect(parseKeyword(keyword)).toBe(expectedResult);
    done();
  });
  ...
  it("should parse all fields criteria as multiple fields with points priority", (done) => {
    ...
    const expectedResult =
      "(name:(2018~ yatch~ race~))^3 OR (start_city:(2018~ yatch~ race~))^2 OR (start_country:(2018~ yatch~ race~))^2 OR (boat_names:(2018~ yatch~ race~))^2 OR (boat_models:(2018~ yatch~ race~))^2 OR (handicap_rules:(2018~ yatch~ race~))^2 OR (source:(2018~ yatch~ race~))^2 OR (unstructured_text:(2018~ yatch~ race~))^2";
    expect(parseKeyword(keyword)).toBe(expectedResult);
    done();
  });
